...
function bepf_theme_styles_and_scripts()
{
	...
	if (!is_resources_page(get_page_template_slug())) {
		...
		wp_enqueue_style('bootstrap', BEPF_REL_PATH . '/lib/css/ecs.min.css', array(), '1.0', 'all');
		wp_enqueue_style('wpbs-style', BEPF_REL_PATH . '/style.css', array(), '1.0', 'all');
		...
		wp_enqueue_style('webfont-roboto', 'https://fonts.googleapis.com/css2?family=Roboto+Slab:wght@400;500&display=swap&subset=cyrillic', array(), '1.0', 'all');
		wp_enqueue_style('webfont-montserrat', 'https://fonts.googleapis.com/css2?family=Montserrat:wght@500&subset=cyrillic', array(), '1.0', 'all');
		wp_enqueue_style(
			'bootstrap',
